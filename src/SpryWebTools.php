...
class SpryWebTools
{
    ...
    /**
     * Initial Loader of the WebTools
     *
     * @return void
     */
    ...
    {
        $enabled = !empty(Spry::config()->webtoolsEnabled);

        if (!$enabled) {
            return false;
        }

        $endpoint = !empty(Spry::config()->webtoolsEndpoint) ? trim(Spry::config()->webtoolsEndpoint) : '';
        $username = !empty(Spry::config()->webtoolsUsername) ? Spry::config()->webtoolsUsername : '';
        $password = !empty(Spry::config()->webtoolsPassword) ? Spry::config()->webtoolsPassword : '';
        $ips = !empty(Spry::config()->webtoolsAllowedIps) ? Spry::config()->webtoolsAllowedIps : [];

        // Check for Values
        if ('/'.trim($endpoint, '/').'/' !== Spry::getPath() ||
            !$username ||
            !$password ||
            !$endpoint ||
            empty($ips) ||
    ...
    private static function displayWebTools()
    {
        if (!empty($_POST['ajax'])) {
            ...
            switch ($ajax) {
                ...
                case 'db_migrate':
                    ...
                    Spry::sendResponse($results);
                    exit;

                    break;

                case 'php_logs':
                    die((file_exists(Spry::config()->logger['php_file']) ? file_get_contents(Spry::config()->logger['php_file']) : ''));

                    break;

                case 'api_logs':
                    die((file_exists(Spry::config()->logger['api_file']) ? file_get_contents(Spry::config()->logger['api_file']) : ''));

                    break;

                case 'clear_php_logs':
                    if (file_exists(Spry::config()->logger['php_file'])) {
                        file_put_contents(Spry::config()->logger['php_file'], '');
                    }

                    break;

                case 'clear_api_logs':
                    if (file_exists(Spry::config()->logger['api_file'])) {
                        file_put_contents(Spry::config()->logger['api_file'], '');
                    }
        ...
        if (!empty($_GET['clear_log']) && !empty(Spry::config()->logger['php_file'])) {
            file_put_contents(Spry::config()->logger['php_file'], '');
        ...
        *:active {
        ...
        ! function(a, b) {
            ...
            f.style.transform === b && a.each(e, function(a, e) {
                if (f.style[a + "TransitionProperty"] !== b) return c = "-" + a.toLowerCase() + "-", d = e, !1
                ...
                return y = function(b) {
                    ...
                    A || y.call(x)
                }, 1e3 * (d + s) + 25)), this.size() && this.get(0).clientLeft, this.css(u), d <= 0 && setTimeout(function() {
                    x.each(function() {
                        y.call(this)
                    })
                }, 0), this
                    ...
                    function(response) {
                        if (response) {
                            ...
                            if (typeof(data.body) !== 'undefined') {
                                ...
                                var testId = $('#api-request-test').val();

                                if (testId == 'All Tests') {
                                    track_submitted_tests('All');
                                } else {
                                    track_submitted_tests(testId);
                                }
                            }
                        }
                    });
                            ...
                            <div class="content">
                                <span class="select">
                                    <select id="api-request-test" onchange="update_json();">
                                        ...
                                        <?php foreach (Spry::config()->tests as $testId => $test) {?>
                                        <option value="<?php echo $testId; ?>"><?php echo $testId.(!empty($test['label']) ? ' - '.$test['label'] : ''); ?></option>
                                        <?php }?>
                                    </select>
                                </span>
                                <span class="submit">
